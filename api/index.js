...
app.get('/api/related', async (req, res) => {
    try {
        const { q, excludeChannel, page = 1 } = req.query;

        if (!q) {
            return res.status(400).json({ error: 'Query parameter "q" is required' });
        }

        console.log(`[Kids API] 🔍 Fetching related videos for: ${q}, excluding: ${excludeChannel}`);

        const pageNum = parseInt(page);
        let searchResults;

        // Search for similar content
        // We add "cartoon kids" to ensure safety and relevance
        const searchQuery = `${q} cartoon kids`;

        if (pageNum === 1) {
            // Fetch 50 results to have enough after filtering out the current channel
            searchResults = await youtubeSearch.GetListByKeyword(searchQuery, false, 50);

            if (searchResults.nextPage) {
                const cacheKey = `${q}_related_continuation`;
                searchCache.set(cacheKey, searchResults.nextPage);
            }
        } else {
            const cacheKey = `${q}_related_continuation`;
        ...
        // Filter videos:
        // 1. Must be a video
        // 2. Must NOT be from the excluded channel
        // 3. Must be 2-20 mins
        const relatedVideos = (searchResults.items || [])
            .filter(item => {
                ...
                if (excludeChannel) {
                    const itemChannel = (item.channelTitle || '').toLowerCase();
                    const excluded = excludeChannel.toLowerCase();
                    if (itemChannel.includes(excluded) || excluded.includes(itemChannel)) {
                        return false;
                    }
                }
                return true;
        ...
        console.log(`[Kids API] Found ${relatedVideos.length} related videos`);
        ...
        console.error('[Kids API] Related videos error:', error);
        res.status(500).json({
